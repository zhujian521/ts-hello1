/**
 * 时间工具模块
 */
export module TimeUtilsModule {
  /**
   * 时间戳转换时间
   * @param timeStamp 10位或者13位数字时间戳
   * @param format 日期格式
   */
  export function timeStampToTime(
    timeStamp: number,
    format: string = "Y-M-D H:m:s"
  ): string {
    var formateArr = ["Y", "M", "D", "H", "m", "s"];
    var returnArr: Array<any> = [];
    let timeStampString = timeStamp.toString();
    timeStamp = timeStampString.length > 10 ? timeStamp : timeStamp * 1000;
    var date = new Date(timeStamp);
    returnArr.push(date.getFullYear());
    returnArr.push(formatNumber(date.getMonth() + 1));
    returnArr.push(formatNumber(date.getDate()));
    returnArr.push(formatNumber(date.getHours()));
    returnArr.push(formatNumber(date.getMinutes()));
    returnArr.push(formatNumber(date.getSeconds()));
    for (var i in returnArr) {
      format = format.replace(formateArr[i], returnArr[i]);
    }
    return format;
  }
  /**
   * 时间转时间戳
   * @param time 日期
   */
  export function timeToTimeStamp(time: string): number {
    return new Date(time.replace(/-/g, "/")).getTime() / 1000;
  }
  /**
   * 计算时间戳之间的差值
   * @param startTime
   * @param endTime
   */
  export function calculateDiffTimeStamp(startTime: number, endTime: number) {
    let start_time = 0;
    let end_time = 0;
    if (startTime < endTime) {
      start_time = startTime;
      end_time = endTime;
    } else {
      start_time = endTime;
      end_time = startTime;
    }
    let timeDiff = end_time - start_time;
    let hour = Math.floor(timeDiff / 3600);
    timeDiff = timeDiff % 3600;
    let minute = Math.floor(timeDiff / 60);
    timeDiff = timeDiff % 60;
    let second = timeDiff;
    return [hour, minute, second];
  }
  /**
   * 计算两个日期之间的差值
   * @param startTime
   * @param endTime
   */
  export function calculateDiffTime(startTime: string, endTime: string) {
    return calculateDiffTimeStamp(
      timeToTimeStamp(startTime),
      timeToTimeStamp(endTime)
    );
  }
  /**
   * 获取当前时间
   */
  export function getCurrentTime() {
    let timestamp = Date.parse(new Date().toString());
    return timeStampToTime(timestamp);
  }

  function formatNumber(n: any) {
    n = n.toString();
    return n[1] ? n : "0" + n;
  }
}
